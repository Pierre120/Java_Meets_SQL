...
-- Table `ET_DB`.`session`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ET_DB`.`session` ;

CREATE TABLE IF NOT EXISTS `ET_DB`.`session` (
  `user_id` BIGINT UNSIGNED NOT NULL,
  `last_login_dttm` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `last_logout_dttm` DATETIME NULL,
  `create_dttm` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`user_id`),
...
GRANT SELECT ON TABLE `ET_DB`.`session` TO 'client';
...
GRANT INSERT ON TABLE `ET_DB`.`session` TO 'client';
...
GRANT UPDATE, DELETE ON TABLE `ET_DB`.`session` TO 'client';
...
START TRANSACTION;
USE `ET_DB`;
INSERT INTO `ET_DB`.`category` (`id`, `name`) VALUES (DEFAULT, 'Food');
INSERT INTO `ET_DB`.`category` (`id`, `name`) VALUES (DEFAULT, 'Drink');
